...
recursive(paths.cordovaBuild, (err, fileNames) => {
...
function printFileSizes(stats, previousSizeMap) {
  var assets = stats.toJson().assets
    ...
    .map(asset => {
      var fileContents = fs.readFileSync(paths.cordovaBuild + '/' + asset.name);
      var size = gzipSize(fileContents);
      var previousSize = previousSizeMap[removeFileNameHash(asset.name)];
      var difference = getDifferenceLabel(size, previousSize);
      return {
        folder: path.join('cordova-build', path.dirname(asset.name)),
...
function build(previousSizeMap) {
  console.log('Creating an optimized production build for Cordova...');
  ...
  compiler.run((err, stats) => {
    ...
    if (publicUrl && publicUrl.indexOf('.github.io/') !== -1) {
      ...
      var build = path.relative(process.cwd(), paths.cordovaBuild);
